...
install_pkg() {
  ...
  [ $? -ne 0 ] && abort "! Cannot install the manager app"
}
...
then
  ...
  do
    # if path detected in user then install the manager app for it
    if pm path --user $user org.frknkrc44.hma_oss &> /dev/null
    then
